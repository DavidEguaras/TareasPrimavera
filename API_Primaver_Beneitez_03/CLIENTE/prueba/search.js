...
const SERVER = `http://${dirIP_api}:${PUERTO_EXPRESS}`;
    ...
    function mostrarResultados(datosFiltrados) {
        ...
        datosFiltrados.forEach(element => {
            ...
            <tr>
                ...
                <td><button type="button" onclick="modificar(${element.id})">Modificar</button></td>
                <td><button type="button" onclick="eliminar(${element.id})">Eliminar</button></td>
                </form>
            </tr>
            `;
        });
...
async function modificar(id) {
    ...
    const actualizado = await updateData(id, nombre, cantidad);
    if (actualizado) {
